...
### Вариант 1: Через Web-интерфейс Tenda (рекомендуется для рабочих роутеров)

1. Скачать стоковую прошивку Tenda с tendacn.com
2. Зайти в web-интерфейс Tenda (обычно 192.168.0.1)
3. Найти раздел "Обновление прошивки" (Firmware Upgrade)
4. Загрузить стоковую прошивку

**Внимание:** Через web можно установить только стоковую прошивку Tenda!

### Вариант 2: Через sysupgrade (для обновления уже установленного OpenWrt)

```bash
# Скачать прошивку
wget https://github.com/ВАШ_РЕПОЗИТОРИЙ/releases/latest/openwrt-*-tenda_rx12l_pro-squashfs-sysupgrade.bin
...
### Вариант 3: Через TFTP (если роутер не загружается)
...
### Вариант 4: Через UART (для кирпича)
